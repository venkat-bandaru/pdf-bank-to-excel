...
class RbsBankExtractor:
    ...
    def summary(
        ...
        """Return RBS's printed summary totals, or None if not found.

        Implements the optional SummaryProvider protocol so normalize.py can
        reconcile the extracted rows. RBS prints the opening ("Previous
        Balance"), total paid in, and closing ("New Balance"); it does not
        print a paid-out total, which reconcile derives from the rows.
        """
        if page_texts is None:
            page_texts = _read_pdf_text(pdf_path)
...
def _parse_summary(text: str) -> RawSummary | None:
    """Pull RBS's Previous Balance / Paid In / New Balance figures."""
